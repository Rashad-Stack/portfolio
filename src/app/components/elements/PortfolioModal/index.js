...
import { ProjectDetails, ScrollableImage, PopUpWrapper } from '../';

const PortfolioModal = ({ onClose, popupData }) => {
  const [open, setOpen] = useState(false);
  const [openImage, setOpenImage] = useState(null);
  const { title, description, type, tags, link, source, video, images } =
    popupData || {};
  ...
  return (
    <>
      <div className="relative h-full w-full flex justify-center items-center bg-black flex-col rounded-md p-3">
        {/* ==== Close Modal ==== */}
        <button
          className="absolute right-5 top-5 text-red-500 text-3xl z-10 outline-none"
          onClick={onClose}
        >
          <AiOutlineCloseCircle />
        </button>
        ...
        <div className="w-full max-h-[698px] h-[90vh] p-7 overflow-y-scroll no-scrollbar">
          ...
          <div className="md:columns-2 mt-10 space-y-5">
            <ProjectDetails icon={<AiTwotoneAppstore />} text={type} />
            <ProjectDetails icon={<AiOutlineTags />} text={tags?.join(', ')} />
            <ProjectDetails
          ...
          {/* ==== Project Video/Image ==== */}
          <div className={`relative w-full max-h-96 rounded-md mt-10 `}>
            {!!video ? (
      ...
      <PopUpWrapper open={open} onClose={onClosePopup}>
        <div className="h-[95vh] w-[80vw] no-scrollbar overflow-y-scroll rounded-md">
